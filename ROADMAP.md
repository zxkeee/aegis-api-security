...
| P1-5 | **Модели развёртывания** 🟡 | Сделано: **observe / pilot режим** (`observe: true`) — гарантированно-безопасная inline-посадка для первого пилота: `ApplyObserveMode` коэрсит всю security-конфигурацию (после Validate, на старте и hot-reload) в «инспектируй и записывай, но ничего не блокируй / не модифицируй / никогда не fail-closed». WAF→DetectionOnly (built-in `deny`-правила тоже, не только CRS), DLP классифицирует PII но не редактирует тело, JWT soft (identity из валидного токена, иначе pass-through, без 401), abuse/schema detect-only, чисто-enforcement контроли выключены, per-route rate-limit override снят. Юнит-тесты (config/DLP/JWT) + живая проверка на бинаре (SQLi/XSS/no-JWT/rate-limit → 200 pass-through, PII не редактируется, enforce всё ещё блокирует). Док `docs/pilot-mode.md`. Дальше: **out-of-band** (зеркалирование трафика/eBPF, снимает inline-хоп) + интеграция с Kong/Apigee/AWS API GW. |
